...
- **User orphan marker: `googleId IS NULL AND bambooId IS NULL`.** The BambooHR sync now pre-creates User rows for staff before they sign in — these rows have `googleId = null` (filled in on first OAuth sign-in) but have `bambooId` set. So `googleId IS NULL` alone no longer means "deletable junk" — it just means the person hasn't signed in yet. The correct query to find true orphans (no Google sign-in, not from Bamboo) is `WHERE "googleId" IS NULL AND "bambooId" IS NULL`. **Never `DELETE FROM "User" WHERE "googleId" IS NULL`** — that would wipe every pre-populated staff member.
